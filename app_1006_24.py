        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            if not filtered_df1.empty:
                ...
                if all_zeros1 and duct_connection_diameter_col in filtered_df1.columns and pd.notna(filtered_df1[duct_connection_diameter_col].values[0]):
                    ...
                    for t in theta:
                        chart_data_3.append({
                            'X_coord_actual': radius1 * np.cos(t),
                            'Y_coord_actual': radius1 * np.sin(t),
                            'Display_Label': f"Left: {selected_year1}-{selected_quarter1}-{selected_brand1}-{selected_unit1}-{selected_size1}",
                            'Point_Order': 0 # For circles, order doesn't matter as much, but keep consistent
                        })
                    chart_data_3.append({ # Close the circle
                        'X_coord_actual': radius1 * np.cos(0),
                        'Y_coord_actual': radius1 * np.sin(0),
                        'Display_Label': f"Left: {selected_year1}-{selected_quarter1}-{selected_brand1}-{selected_unit1}-{selected_size1}",
                        'Point_Order': 0
                    })
            ...
            if not filtered_df2.empty:
                ...
                if all_zeros2 and duct_connection_diameter_col in filtered_df2.columns and pd.notna(filtered_df2[duct_connection_diameter_col].values[0]):
                    ...
                    for t in theta:
                        chart_data_3.append({
                            'X_coord_actual': radius2 * np.cos(t),
                            'Y_coord_actual': radius2 * np.sin(t),
                            'Display_Label': f"Right: {selected_year2}-{selected_quarter2}-{selected_brand2}-{selected_unit2}-{selected_size2}",
                            'Point_Order': 0 # For circles, order doesn't matter as much, but keep consistent
                        })
                    chart_data_3.append({ # Close the circle
                        'X_coord_actual': radius2 * np.cos(0),
                        'Y_coord_actual': radius2 * np.sin(0),
                        'Display_Label': f"Right: {selected_year2}-{selected_quarter2}-{selected_brand2}-{selected_unit2}-{selected_size2}",
                        'Point_Order': 0
                    })
