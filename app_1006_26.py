    ...
    for col in df.columns:
        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            if not filtered_df1.empty:
                ...
                # First, ensure columns exist before trying to access values
                all_coords_present_and_zero_or_na1 = True
                for c in required_coord_cols_chart3:
                    if c not in filtered_df1.columns or (pd.notna(filtered_df1[c].values[0]) and filtered_df1[c].values[0] != 0):
            ...
            if not filtered_df2.empty:
                ...
                for c in required_coord_cols_chart3:
                    if c not in filtered_df2.columns or (pd.notna(filtered_df2[c].values[0]) and filtered_df2[c].values[0] != 0):
