...
header_triggers_raw = {
    ...
    "Duct connection Diameter": "Rotary wheel", # Correctly triggers this header
...
with col_filter1:
    ...
    filtered_df1 = df_filtered_by_recovery1[df_filtered_by_recovery1[size_col] == selected_size1]
...
with col_filter2:
    ...
    filtered_df2 = df_filtered_by_recovery2[df_filtered_by_recovery2[size_col] == selected_size2]
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    for col in df.columns:
        ...
        # We need to get the *actual* column name that corresponds to the raw trigger string
        # This prevents issues if the Excel column name has slight variations that get_column_safe handles
        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            if not filtered_df1.empty:
                # Check if all X11-Y15 are 0
                all_zeros1 = all(filtered_df1[c].values[0] == 0 for c in required_coord_cols_chart3 if c in filtered_df1.columns)
                if all_zeros1 and duct_connection_diameter_col in filtered_df1.columns and pd.notna(filtered_df1[duct_connection_diameter_col].values[0]):
                ...
                elif not all_zeros1: # If not all zeros, plot rectangle
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_11_15):
                        if x_name in filtered_df1.columns and y_name in filtered_df1.columns and \
                           pd.notna(filtered_df1[x_name].values[0]) and pd.notna(filtered_df1[y_name].values[0]):
                            ...
                            break
                else: # All zeros but diameter missing or invalid
                    can_plot_brand1_chart3 = False
                    st.info(f"Coordinate data (X11-X15, Y11-Y15) for 'Left: {selected_brand1} - {selected_unit1}' is all zeros, but 'Duct connection Diameter' is missing or invalid. Cannot draw circle for Chart 3.")
            else:
            ...
            if not filtered_df2.empty:
                # Check if all X11-Y15 are 0
                all_zeros2 = all(filtered_df2[c].values[0] == 0 for c in required_coord_cols_chart3 if c in filtered_df2.columns)
                if all_zeros2 and duct_connection_diameter_col in filtered_df2.columns and pd.notna(filtered_df2[duct_connection_diameter_col].values[0]):
                ...
                elif not all_zeros2: # If not all zeros, plot rectangle
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_11_15):
                    ...
                    st.info(f"Coordinate data (X11-X15, Y11-Y15) for 'Right: {selected_brand2} - {selected_unit2}' is all zeros, but 'Duct connection Diameter' is missing or invalid. Cannot draw circle for Chart 3.")
            else:
                can_plot_brand2_chart3 = False
