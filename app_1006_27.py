...
duct_connection_height_col = get_column_safe(df, ["Duct connection Height", "Duct Connection Height"])
# Column for circle diameter
duct_connection_diameter_col = get_column_safe(df, ["Duct connection Diameter", "Duct Connection Diameter"])

# New columns for dropdowns and chart
type_col = get_column_safe(df, ["Type"])
material_col = get_column_safe(df, ["Material"])
capacity_range1_col = get_column_safe(df, ["Capacity range1", "Capacity Range1"])
capacity_range2_col = get_column_safe(df, ["Capacity range2", "Capacity Range2"])
capacity_range3_col = get_column_safe(df, ["Capacity range3", "Capacity Range3"])


# Header trigger columns (using potential original strings for mapping)
header_triggers_raw = {
    "Unit size quantity": "Certification data",
    "VDI 6022-1 certification": "Available configurations",
    "SE with glycol": "Casing data",
    "Metal sheet thickness (External)": "Airflows",
    "Air speed on Filter at max airflow (ErP)": "Dimensions",
    "Duct connection Diameter": "Rotary wheel",
    "Efficiency at max balanced airflows": "PCR/HEX recovery exchanger",
    "Efficiency at max balanced airflows.1": "Fan section data",
    "Impeller efficiency at nominal airflow": "Electrical heater",
    "Capacity range3": "Water heater", # This will now be the trigger for Water heater header
    "Water heater_max rows": "Water cooler",
    "Water cooler_max rows": "DX/DXH cooler",
    "Available refrigerant types": "Supply Filter",
    "Final PD_typ1": "Exhaust Filter",
    "Final PD_typ2": "Silencer data"
}

# Resolve actual column names for headers from the DataFrame
resolved_header_triggers = {}
for potential_col_name, header_title in header_triggers_raw.items():
    actual_col_name = get_column_safe(df, [
        potential_col_name,
        potential_col_name.replace(" (", " (").replace(")", ""), # Handle parenthesis variations
        potential_col_name.lower(),
        potential_col_name.title(),
        potential_col_name.replace(" ", "_"),
        potential_col_name.replace(" ", "").lower(),
    ])
    if actual_col_name:
        resolved_header_triggers[actual_col_name] = header_title

# Chart for Electrical Heater Capacity: after the row containing capacity_range3_col
electrical_heater_chart_trigger_col = capacity_range3_col
...
# --- Dropdown Menu Filtering Logic (Cascading) ---
with col_filter1:
    # Year filter
    available_years1 = sorted(df[year_col].dropna().unique())
    selected_year1 = st.selectbox("Year", available_years1, key="year1")
    ...
    selected_quarter1 = st.selectbox("Quarter", available_quarters1, key="quarter1")
    ...
    selected_region1 = st.selectbox("Region", available_regions1, key="region1")
    ...
    selected_brand1 = st.selectbox("Select Brand", available_brands1, key="brand1")
    df_filtered_by_brand1 = df_filtered_by_region1[df_filtered_by_region1[brand_col] == selected_brand1]

    brand1_logo_path = df_filtered_by_brand1[logo_col].iloc[0] if not df_filtered_by_brand1.empty and logo_col else None
    
    # Unit name filter
    available_units1 = sorted(df_filtered_by_brand1[unit_name_col].dropna().unique())
    selected_unit1 = st.selectbox("Unit name", available_units1, key="unit1")
    ...
    selected_recovery1 = st.selectbox("Recovery type", available_recovery_types1, key="recovery1")
    ...
    selected_size1 = st.selectbox("Unit size", available_sizes1, key="size1")
    df_filtered_by_size1 = df_filtered_by_recovery1[df_filtered_by_recovery1[size_col] == selected_size1]

    # Type filter (New)
    available_types1 = sorted(df_filtered_by_size1[type_col].dropna().unique()) if type_col else []
    selected_type1 = st.selectbox("Type", available_types1, key="type1") if type_col else None
    df_filtered_by_type1 = df_filtered_by_size1[df_filtered_by_size1[type_col] == selected_type1] if type_col else df_filtered_by_size1

    # Material filter (New)
    available_materials1 = sorted(df_filtered_by_type1[material_col].dropna().unique()) if material_col else []
    selected_material1 = st.selectbox("Material", available_materials1, key="material1") if material_col else None
    df_filtered_by_material1 = df_filtered_by_type1[df_filtered_by_type1[material_col] == selected_material1] if material_col else df_filtered_by_type1

    # This is the final filtered DataFrame for the left column
    filtered_df1 = df_filtered_by_material1


with col_filter2:
    # Year filter
    available_years2 = sorted(df[year_col].dropna().unique())
    selected_year2 = st.selectbox("Year", available_years2, key="year2")
    ...
    selected_quarter2 = st.selectbox("Quarter", available_quarters2, key="quarter2")
    ...
    selected_region2 = st.selectbox("Region", available_regions2, key="region2")
    ...
    selected_brand2 = st.selectbox("Select Brand", available_brands2, key="brand2")
    df_filtered_by_brand2 = df_filtered_by_region2[df_filtered_by_region2[brand_col] == selected_brand2]

    brand2_logo_path = df_filtered_by_brand2[logo_col].iloc[0] if not df_filtered_by_brand2.empty and logo_col else None

    # Unit name filter
    available_units2 = sorted(df_filtered_by_brand2[unit_name_col].dropna().unique())
    selected_unit2 = st.selectbox("Unit name", available_units2, key="unit2")
    ...
    selected_recovery2 = st.selectbox("Recovery type", available_recovery_types2, key="recovery2")
    ...
    selected_size2 = st.selectbox("Unit size", available_sizes2, key="size2")
    df_filtered_by_size2 = df_filtered_by_recovery2[df_filtered_by_recovery2[size_col] == selected_size2]

    # Type filter (New)
    available_types2 = sorted(df_filtered_by_size2[type_col].dropna().unique()) if type_col else []
    selected_type2 = st.selectbox("Type", available_types2, key="type2") if type_col else None
    df_filtered_by_type2 = df_filtered_by_size2[df_filtered_by_size2[type_col] == selected_type2] if type_col else df_filtered_by_size2

    # Material filter (New)
    available_materials2 = sorted(df_filtered_by_type2[material_col].dropna().unique()) if material_col else []
    selected_material2 = st.selectbox("Material", available_materials2, key="material2") if material_col else None
    df_filtered_by_material2 = df_filtered_by_type2[df_filtered_by_type2[material_col] == selected_material2] if material_col else df_filtered_by_type2

    # This is the final filtered DataFrame for the right column
    filtered_df2 = df_filtered_by_material2

# --- Image Height Synchronization ---
# Load and resize brand logos
loaded_image1 = None
if brand1_logo_path:
    try:
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    excluded_cols_from_table = [
        ...
        unit_name_col, recovery_col, size_col, type_col, material_col # Exclude new dropdowns
    ]
    # Add all resolved coordinate column names to the excluded list as they are drawn in charts
    ...
    # Also exclude capacity range columns as they will be in the new bar chart
    if capacity_range1_col: excluded_cols_from_table.append(capacity_range1_col)
    if capacity_range2_col: excluded_cols_from_table.append(capacity_range2_col)
    if capacity_range3_col: excluded_cols_from_table.append(capacity_range3_col)
    ...
    displayed_headers = set() # To track headers already displayed
    ...
    for col in df.columns:
        ...
        if col not in excluded_cols_from_table:
            ...
            with row_col2:
                # Apply center alignment to values
                st.write(f'<div style="text-align: center; font-family: sans-serif; font-size: 16px;">{val1}</div>', unsafe_allow_html=True)
            with row_col3:
                # Apply center alignment to values
                st.write(f'<div style="text-align: center; font-family: sans-serif; font-size: 16px;">{val2}</div>', unsafe_allow_html=True)


        # --- Insert Section Headers (AFTER the trigger row) ---
        # Iterate over the original raw triggers to find the current column
        for raw_trigger_col, header_title in header_triggers_raw.items():
            actual_trigger_col_found = get_column_safe(df, [
                raw_trigger_col,
                raw_trigger_col.replace(" (", " (").replace(")", ""), # Handle parenthesis variations
                raw_trigger_col.lower(),
                raw_trigger_col.title(),
                raw_trigger_col.replace(" ", "_"),
                raw_trigger_col.replace(" ", "").lower(),
            ])
            if actual_trigger_col_found == col and header_title not in displayed_headers:
                st.markdown("---")
                header_col1, header_col2, header_col3 = st.columns([1, 4, 1])
                with header_col2:
                    st.markdown(f'<h4 style="text-align: center; font-size: 1.2em; margin-bottom: 0.5em; margin-top: 0.5em;">{header_title}</h4>', unsafe_allow_html=True)
                ...
                with col3:
                    st.markdown(f'<div style="text-align: center;">**{selected_brand2} - {selected_unit2} - {selected_size2}**</div>', unsafe_allow_html=True)
                displayed_headers.add(header_title) # Mark as displayed
                break # Break from inner loop once header is displayed
        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            if not filtered_df1.empty:
                # Check if all X11-Y15 are 0 or NaN/None (handled by pd.isna)
                ...
                if all_coords_present_and_zero_or_na1 and duct_connection_diameter_col in filtered_df1.columns and pd.notna(filtered_df1[duct_connection_diameter_col].values[0]):
                    ...
                    for t in theta:
                        chart_data_3.append({
                            'X_coord_actual': center_x1 + radius1 * np.cos(t), # Centered circle
                            'Y_coord_actual': center_y1 + radius1 * np.sin(t), # Centered circle
                            'Display_Label': f"Left: {selected_year1}-{selected_quarter1}-{selected_brand1}-{selected_unit1}-{selected_size1}",
                            'Point_Order': 0
                        })
                    ...
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_11_15):
                        if x_name in filtered_df1.columns and y_name in filtered_df1.columns and \
                           pd.notna(filtered_df1[x_name].values[0]) and pd.notna(filtered_df1[y_name].values[0]):
                            ...
                            can_plot_brand1_chart3 = False # Missing some coords for rectangle
                ...
                can_plot_brand1_chart3 = False # No data for filtered_df1

            can_plot_brand2_chart3 = True
            if not filtered_df2.empty:
                # Check if all X11-Y15 are 0 or NaN/None
                ...
                if all_coords_present_and_zero_or_na2 and duct_connection_diameter_col in filtered_df2.columns and pd.notna(filtered_df2[duct_connection_diameter_col].values[0]):
                    ...
                    for t in theta:
                        chart_data_3.append({
                            'X_coord_actual': center_x2 + radius2 * np.cos(t), # Centered circle
                            'Y_coord_actual': center_y2 + radius2 * np.sin(t), # Centered circle
                            'Display_Label': f"Right: {selected_year2}-{selected_quarter2}-{selected_brand2}-{selected_unit2}-{selected_size2}",
                            'Point_Order': 0
                        })
                    chart_data_3.append({ # Close the circle
                    ...
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_11_15):
                        if x_name in filtered_df2.columns and y_name in filtered_df2.columns and \
                           pd.notna(filtered_df2[x_name].values[0]) and pd.notna(filtered_df2[y_name].values[0]):
                            ...
                            can_plot_brand2_chart3 = False # Missing some coords for rectangle
            ...
            if chart_data_3:
                ...
                fig3.update_traces(line=dict(width=1.5)) # Reduced line thickness for circles
                fig3.update_layout(
        ...
        # --- Insert Electrical Heater Capacity Chart after relevant column ---
